...
- The gallery and component-catalog pages iframe real rendered output inside Preview/Code `Tabs`: `pnpm docs:render` renders `examples/*.mdx` → `docs/public/examples/` and `examples/catalog/*.mdx` → `docs/public/components/` (needs `dist/cli.mjs`, so run `pnpm build` first; the docs workflow does both). Every render also emits a `<name>.src.html` sibling — the .mdx source in mdxr's own code block — which the Code tab iframes. The `docs/extended/*.mdx` pages go further: each `## N. Component` section gets its own pair whose Preview iframe loads `extended-<family>.html#<heading-id>`, scrolling the family demo to that component (anchors are taken from the rendered HTML, which dedupes ids like `evidence-1`). `examples/catalog/` holds one demo document per catalog section and carries its own `mdxr.config.ts` + `components/` for the project-defined-components demo.
