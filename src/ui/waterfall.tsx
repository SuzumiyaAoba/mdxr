...
export const Span = defineComponent(
  ...
  ({ name, duration, note, start }) => {
    ...
    const s = numOf(start) ?? 0;
    const d = numOf(duration) ?? 0;
...
const spanDuration = (node: ReactNode): number | undefined => {
  ...
  const duration = numOf(propOf(node, "duration"));
  if (duration === undefined) {
    return undefined;
  }
  return (numOf(propOf(node, "start")) ?? 0) + duration;
};
...
export const Waterfall = defineComponent(
  ...
  ({ title, total, unit, children }) => {
    ...
    const totalNum = numOf(total) ?? maxEnd;
